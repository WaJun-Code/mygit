...
model=LGBMRegressor()
model=joblib.load("lgb_model42.pkl")
...
def calc_spread_return_sharpe(df: pd.DataFrame, portfolio_size: int = 200, toprank_weight_ratio: float = 2) -> float:
    ...
    return sharpe_ratio
test_fin = pd.read_csv("./supplemental_files/financials.csv",low_memory=False)  #各个季度的季度情况
test_op = pd.read_csv("./supplemental_files/options.csv",low_memory=False)
test_sec_pr = pd.read_csv("./supplemental_files/secondary_stock_prices.csv",low_memory=False)
test_pr = pd.read_csv("./supplemental_files/stock_prices.csv",low_memory=False)
test_trd = pd.read_csv("./supplemental_files/trades.csv",low_memory=False).dropna(how='any').reset_index(drop=True)

#可能B榜需要不断concat一个df来临时滑窗，但只取最后一个滑窗结果即可
submit = pd.DataFrame()
for date in tqdm.tqdm(test_pr['Date'].unique()):
    prices,secondary_prices,options = test_pr[test_pr["Date"]==date].reset_index(drop=True),test_sec_pr[test_sec_pr["Date"]==date].reset_index(drop=True),test_op[test_op["Date"]==date].reset_index(drop=True)
    financials,trades = test_fin[test_fin["Date"]==date].reset_index(drop=True),test_trd[test_trd["Date"]==date].reset_index(drop=True)
    sample_prediction = copy.deepcopy(prices[["Date","SecuritiesCode","Target"]])
    #通过df_xy不断粘上新的df
    prices,financials,trades = pd.concat((train_pr,prices)).reset_index(drop=True),pd.concat((train_fin,financials)).reset_index(drop=True),pd.concat((train_trd,trades)).reset_index(drop=True)
    ...
    sample_prediction = sample_prediction.merge(data,how='left',on=["SecuritiesCode"])
    submit = pd.concat((submit,sample_prediction)).reset_index(drop=True)
    
score = calc_spread_return_sharpe(submit, 200, 2)  #新写一个score函数
print("实时滑窗线下得分为:",score)
