...
class LSTM(nn.Module):
    ...
    def forward(self,X,online=False):   #输入一个【bz，seq+2，dim】,可以在这多录入2个，形成seq+2，好加loss
        ...
        out = self.Dense(out[:,-seq+2:]).flatten(0)
        if online:return out
        else:return out,loss1+loss2
LSTM = LSTM().to(device)
LSTM.load_state_dict(torch.load("LSTM42.pth", map_location=device))
LSTM.eval()
...
for date in tqdm.tqdm(test_pr['Date'].unique()):
    ...
    data = copy.deepcopy(sample_prediction[["Date","SecuritiesCode"]])
