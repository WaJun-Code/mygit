...
#model = BertForSequenceClassification.from_pretrained('bert-base-cased', num_labels=13)   #'bert-base-chinese'和'bert-base-cased'
...
class Predictor(nn.Module):
    def __init__(self, encoder, decoder, device, atom_dim=34):
        ...
        self.decoder = decoder   #是一个Transformer
        self.device = device
    ...
    def gcn(self, input, adj):   #通过GCN得到[num_node,atom_dim]的映射向量簇
    ...
    def __call__(self, data, train=True):
        ...
        if train:
            ...
            ys = F.softmax(predicted_interaction, 1).float()
            # loss = Loss(predicted_interaction, correct_interaction)
            loss = F.binary_cross_entropy(ys[:, 1], correct_interaction.float())   #此处选择损失函数
            #loss = loss_fn(ys, torch.tensor(correct_interaction, dtype=torch.int64))
            return loss
            ...
            ys = F.softmax(predicted_interaction, 1).to('cpu').data.numpy()   #在1方向上softmax
...
class Trainer(object):
    ...
    def train(self, file,limit, device):
        self.model.train()
        loss_total = 0
        i = 0
        ...
        with open(file, 'r') as fd:
            for line in tqdm.tqdm(fd.readlines(), total=limit,
                ...
                atom, adj = mol_features(smiles)
                protein = get_protein_vec(protein)
                label =torch.tensor([float(label)])
                adjs.append(adj.to(device))
                atoms.append(atom.to(device))
                proteins.append(protein.to(device))
...
class Tester(object):
    ...
    def test(self, dataset):
        ...
        return AUC, PRC, gather_frac(T, S)
