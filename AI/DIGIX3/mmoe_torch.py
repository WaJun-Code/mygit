...
class focal_loss(nn.Module):     #alpha越大，结果越偏向于该样本
    ...
    def forward(self, preds, labels):
        ...
        preds_logsoft = torch.log(preds+1e-9)
...
class MMoE(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(len(self.sparse)):
            ...
            if encoder_inputs == None:encoder_inputs = emb.unsqueeze(1) # [batch_size, 1, 32]
            else:encoder_inputs = torch.cat((encoder_inputs, emb.unsqueeze(1)), dim = 1) # 沿着seq_len的维度拼接
        ...
        # 按照txt中的方法，encoder_inputs应为[batch_size, 9, 32]
        if encoder_inputs!=None:
            encoder_outputs = encoder_inputs
            encoder_outputs = encoder_outputs.reshape([encoder_outputs.shape[0], encoder_outputs.shape[1]*encoder_outputs.shape[2]]) # [batch_size, 9*32]
            inputs = torch.cat((inputs, encoder_outputs), dim = -1) # [batch_size, self.input_dim = 288]
        inputs= inputs.float()
...
def multi_PRC(label, pre):   #label是一维，pre是n维
    Prec1, Recall1 = 0,0
    for i in range(1,pre.shape[1]):  #0标签不算分
        t,y = label[:,i],pre[:,i]
...
def submit(test_data,batch_size=512):
    ...
    np.save("watch1.npy",watchlist)
    np.save("share1.npy",sharelist)
...
if __name__ == '__main__':
    ...
    del data,train_data,val_data,batch   #需删掉全部浅复制
    gc.collect()
